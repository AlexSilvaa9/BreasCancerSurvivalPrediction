...
Flow diagram for the model building process:
...
digraph {
  layout=dot;
  node [shape=circle, style=filled, fillcolor=\"#E2BBE9\", fontname=\"Arial\", fontsize=90, tooltip=\"Haz clic para más detalles\",fixedsize = FALSE];
  edge [color=black, arrowhead=vee];
  
  Data_Cleaning -> Statistical_Analysis;
  Statistical_Analysis -> Model_Comparison;
  Model_Comparison -> Logistic_Regression;
  Model_Comparison -> Neural_Networks;
  Model_Comparison -> Support_Vectors_Machine;
  Model_Comparison -> Decision_Tree;
  Model_Comparison -> Random_Forest;
  Model_Comparison -> Naive_Bayes;
  Model_Comparison -> K_Nearest_Neighbors;
  Model_Comparison -> AdaBoost;
  Model_Comparison -> Best_Model;
  Best_Model -> Threshold_Optimization;
  Best_Model -> Best_Variables_Selection;
  Best_Variables_Selection -> Deployment;
  Threshold_Optimization -> Deployment;
  Model_Comparison -> Data_Cleaning;
}
"

# Crear el gráfico
graph <- grViz(gr,width = 1200, height = 800)
graph
```
...
<div style="max-width: 600px; margin: 0 auto;">
    <a href="diagrama.png" data-lightbox="imagen" data-title="Diagram">
        <img src="diagrama.png" style="width: 100%; height: auto; border: 2px solid #ddd; border-radius: 5px;" alt="Diagram">
        <p style="text-align: center; margin-top: 5px; font-size: 14px;"><em>Click to enlarge</em></p>
    </a>
</div>
<br>
...
crear_comparacion_metricas <- function(lista_tablas_metricas, nombres_tablas) {
  ...
  plot <- ggplot(tabla_metricas_long, aes(x = Modelo, y = Valor, fill = Metrica)) +
    ...
    labs(title = "Metrics comparation btween models", x = "Model", y = "Value") +
    theme_minimal() +
    scale_fill_manual(values = c("AUC" = "steelblue", "Accuracy" = "lightblue", "Recall" = "#3C5B6F", "Specificity" = "purple"), name = "Métrica") +
